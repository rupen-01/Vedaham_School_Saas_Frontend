import { useState, useEffect } from "react";
import { X } from "lucide-react";
import Card from "../../../components/common/Card";
import SlidingLabelInput from "../../../components/common/SlidingLabelInput";
import SlidingLabelTextarea from "../../../components/common/SlidingLabelTextarea";
import ToggleSwitch from "../../../components/common/ToggleSwitch";
import { useMutationApi } from "../../../config/UseApi";
import Pop from "../../../components/common/pop";
import ApiConfig from "../../../config/ApiConfig";

const CreateLeaveType = ({ isOpen = true, onClose = () => {}, editData = null }) => {
  const [leaveTypeData, setLeaveTypeData] = useState({
    name: "",
    description: "",
    max_days: "",
    status: true,
  });

  // Populate form if editing
  useEffect(() => {
    if (editData) {
      setLeaveTypeData({
        name: editData.name || "",
        description: editData.description || "",
        max_days: editData.max_days || "",
        status: editData.status ?? true,
      });
    } else {
      setLeaveTypeData({
        name: "",
        description: "",
        max_days: "",
        status: true,
      });
    }
  }, [editData]);
// ---------------------------
// API Mutation (Leave Type)
// ---------------------------
const mutation = useMutationApi({
  key: "leave-types",
  url: editData
    ? ApiConfig.LEAVE_TYPE_UPDATE(editData.id)
    : ApiConfig.LEAVE_TYPE_CREATE,
  method: editData ? "PUT" : "POST",
  requireAuth: true,
  options: {
    onSuccess: () => {
      Pop(
        "success",
        editData ? "Leave Type updated successfully!" : "Leave Type created successfully!"
      );
      onClose();
      setLeaveTypeData({
        name: "",
        description: "",
        max_days: "",
        status: true,
      });
    },
    onError: (err) => {
      console.error("Failed:", err);
      Pop("error", err?.response?.data?.message || "Something went wrong.");
    },
  },
});

if (!isOpen) return null;

const handleChange = (field, value) => {
  setLeaveTypeData((prev) => ({ ...prev, [field]: value }));
};

  const handleSubmit = () => {
    const dataToSend = { ...leaveTypeData, max_days: Number(leaveTypeData.max_days) };
    console.log("Creating/Updating Leave Type:", dataToSend);
    mutation.mutate({ payload: dataToSend });
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-3 sm:px-6 overflow-y-auto">
      <div className="w-full max-w-md sm:max-w-2xl mx-auto my-6">
        <Card className="p-4 sm:p-6 mt-4">
          {/* Header */}
          <div className="flex justify-between items-center pb-3 mb-4 border-b border-gray-200">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800">
              {editData ? "Edit Leave Type" : "Add Leave Type"}
            </h2>
            <button
              onClick={onClose}
              className="text-red-500 hover:text-red-700 bg-red-100 w-8 h-8 flex items-center justify-center rounded-lg transition cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>

          {/* Form Fields */}
          <div className="flex flex-col gap-5">
            <SlidingLabelInput
              type="text"
              label="Leave Name"
              placeholder="Medical Leave"
              value={leaveTypeData.name}
              onChange={(e) => handleChange("name", e.target.value)}
            />
            <SlidingLabelTextarea
              label="Description"
              placeholder="Medical and health related leave"
              value={leaveTypeData.description}
              onChange={(e) => handleChange("description", e.target.value)}
            />
            <SlidingLabelInput
              type="number"
              label="Maximum Days"
              placeholder="30"
              value={leaveTypeData.max_days}
              onChange={(e) => handleChange("max_days", e.target.value)}
            />
          </div>

          {/* Status Toggle */}
          <div className="mt-4">
            <ToggleSwitch
              label="Status"
              name="status"
              checked={leaveTypeData.status}
              onChange={(value) => handleChange("status", value)}
            />
          </div>

          {/* Submit Button */}
          <div className="flex justify-end mt-6">
            <button
              onClick={handleSubmit}
              disabled={mutation.isLoading}
              className={`px-5 sm:px-6 py-2 rounded-lg text-white cursor-pointer ${
                mutation.isLoading ? "bg-gray-400 cursor-not-allowed" : "bg-indigo-600 hover:bg-indigo-700"
              }`}
            >
              {mutation.isLoading ? (editData ? "Updating..." : "Creating...") : editData ? "Update Leave Type" : "Create Leave Type"}
            </button>
          </div>
        </Card>
      </div>
    </div>
  );
};

export default CreateLeaveType;
