  ...
  return (
    <div className="flex items-center justify-between shadow-md px-7 py-3 bg-white/80 relative">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="hover:rounded-md hover:shadow-md cursor-pointer flex items-center gap-2.5 px-1.5 py-[3px]" onClick={() => setOpen((v) => !v)}>
        <div className="relative">
          <div
            className="w-11 h-11 rounded-full bg-gray-100 flex items-center cursor-pointer justify-center shadow border-2 border-transparent transition"
          >
            <img
              src={user.avatar}
              alt="Profile Image"
          ...
          {/* {open && (
            <div
              ref={dropdownRef}
              className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 flex flex-col z-50"
            >
              <div className="flex flex-col items-center px-6 pt-6 pb-3">
                <img
                  src={user.avatar}
                  alt="Profile"
                  className="w-12 h-12 rounded-full object-cover object-center border-2 border-indigo-200 mb-2"
                />
                <div className="font-semibold text-gray-900 text-sm">{user.name}</div>
                <div className="text-gray-500 text-xs mb-1">{user.role}</div>
              </div>
              <div className="flex-1" />
              <div className="border-t border-gray-100 flex flex-col">
                <button className="w-full text-left text-sm px-6 py-3 cursor-pointer hover:bg-gray-50 text-gray-700 font-medium">Settings</button>
                <button className="w-full text-left text-sm px-6 py-3 cursor-pointer hover:bg-gray-50 text-red-500 font-medium">Sign Out</button>
              </div>
            </div>
          )} */}
          ...
          {open && (
            ...
            >
              <div className="flex items-center px-3 gap-x-3 pt-6 pb-3">
                ...
                />
                <div>
                <div className="font-semibold text-left text-gray-900 text-sm">{user.name}</div>
                <div className="text-gray-500 text-left text-xs mb-1">{user.role}</div>
                </div>
              </div>
              <div className="flex-1" />
              <div className="border-t border-gray-300 flex flex-col">
                <button className="w-full text-left px-6 py-2.5 cursor-pointer text-sm hover:bg-gray-50 text-gray-700 font-medium">Settings</button>
                <button onClick={handleSignOut} className="w-full text-left px-6 py-2.5 cursor-pointer text-sm hover:bg-gray-50 text-red-500 font-medium">Sign Out</button>
              </div>
            </div>
          )}
        </div> 
